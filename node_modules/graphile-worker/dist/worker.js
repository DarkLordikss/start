...
function makeNewWorker(options, tasks, withPgClient, continuous = true) {
    const { workerId = `worker-${crypto_1.randomBytes(9).toString("hex")}`, pollInterval = config_1.defaults.pollInterval, } = options;
    const { workerSchema, escapedWorkerSchema, logger, maxContiguousErrors, } = lib_1.processSharedOptions(options, {
        scope: {
            label: "worker",
            workerId,
        },
    ...
    const doNext = async () => {
        ...
        try {
            ...
            try {
                ...
                const helpers = helpers_1.makeJobHelpers(options, job, { withPgClient, logger });
